...
const Select = ({cnSelect ,label , cnInput , dataAos , required , error , trigger , watch , KEY, setValue, icon, data, cn, place }) => {
  ...
  return (
    <div ref={selectRef} data-aos={dataAos}  className={`${cn} z-[100000] relative flex  flex-col gap-[15px]  w-full `}>
      {label && <label htmlFor={KEY} className={` font-[400] text-[14px] text-[#333333] `} > {label} {required && <span className='text-[#ff4b55]' > * </span> } </label> }

      <div onClick={() => setShow(!show)} className={` ${cnSelect} z-[1000] relative px-[10px] border-[#e5e5e5] outline-none border duration-300 rounded-[8px] cursor-pointer w-full h-[59px] flex justify-between items-center`}>
        ...
        <div style={{direction:"ltr"}} className={`almarai ${show ? 'opacity-100 !pointer-events-auto' : 'opacity-0 pointer-events-none '} z-[1000000]  max-h-[200px] overflow-auto  duration-300 shadow-md ease-in-out absolute top-[110%] w-full bg-white `}>
          {data.map((e, i) => (
