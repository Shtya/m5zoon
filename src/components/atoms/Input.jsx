...
export default function Input({ required , delay , place , dataAos , error , cnInput , cn , label , type , icon , KEY , register , cnLabel}) {
  ...
  const toggleInputType = () => {
	...
	<div data-aos={dataAos} data-aos-delay={delay} className={`${cn} flex flex-col  gap-[15px] max-md:gap-[10px]  relative `} >
		{label && <label htmlFor={KEY} className={`${cnLabel} text-black/60 t22 max-md:t18 font-normal `} > {label} <span className='text-[#ff4b55]' > {required && "*"} </span> </label> }
		
		<div  className={`h-[60px]  relative overflow-hidden rounded-[8px] text-xl max-md:t16 border border-[#7d7d7d]/20 w-full ${cnInput} `} >
			<input className={`placeholder:text-[#979797]/60 text-[#555]  font-normal  w-full ${icon ? "ltr:pr-[40px] rtl:pl-[40px]" : "" } px-[10px] h-full outline-none `} {...register} id={KEY} placeholder={place} type={inputType} />
