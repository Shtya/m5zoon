            ...
            <div className='max-md:!w-[100%]  max-md:mx-auto  max-md:!right-[unset] max-md:left-0 absolute h-[100vh]  duration-300 w-[50%] top-0 ltr:left-0  rtl:right-0 z-[1000]   '>
				<div className='max-md:!max-w-full bg-opacity-[.95] backdrop-blur-2xl bg-white rtl:mr-auto ltr:ml-auto max-w-[700px] w-full relative max-lg:p-[20px]  p-[50px]  h-[100vh]  overflow-auto ' >
                    ...
                    <div className='flex text-center justify-between mt-[30px] ' >
                        <Checkbox cnLabel={"w-full"} cn={"mt-0"} KEY={"terms"} setValue={setValue} watch={watch}  label={termsLabel} />
                        <Link href={"/forget-password"} className='text-[#f9a51a] text-lg font-normal underline' > {t("forgot_password")} </Link>
                    </div>

					<Button name={t("login")} cn={"  !max-w-[670px] !w-full mx-auto !my-[50px] hover:bg-opacity-90  bg-secondery "} />

					<p className='text-center text-[#09396f] text-2xl font-normal'>  {t("no_account")} <Link className='text-[#f9a51a]'  href={"/sign-up"} > {t("join_us_now")} </Link>  </p>
